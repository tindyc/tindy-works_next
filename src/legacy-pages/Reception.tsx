...
function getDescriptionError(value: string) {
  ...
  if (trimmed.length < 10) {
    return 'Please add a little more detail so I can understand the problem.';
  }

  if (['test', 'asdf', 'qwerty', 'hello'].includes(simplified)) {
    return 'Please describe the real issue you need help with.';
  }

  if (/^(.)\1{4,}$/u.test(simplified) || new Set(simplified).size <= 1) {
    return 'Please use a few words that explain what is going wrong.';
  }
...
function getContactValueError(method: ContactMethod | undefined, value: string) {
  if (!method) {
    return 'Please choose how you would like me to contact you.';
  }

  if (!value.trim()) {
    return method === 'phone' ? 'Please enter a phone number.' : 'Please enter an email address.';
  }
...
function getValidationErrors({
  ...
  if (!contactMethod) {
    errors.contactMethod = 'Please choose phone call or email.';
  }

  const contactValueError = getContactValueError(contactMethod, contactValue);
  if (contactValueError) {
    errors.contactValue = contactValueError;
  }
  ...
  if (isOnBehalf) {
    if (!otherPerson.name.trim()) {
      errors.otherPersonName = 'Please enter the name of the person who needs help.';
    }

    if (!otherPerson.relationship.trim()) {
      errors.relationship = 'Please choose your relationship to them.';
    }

    if (otherPerson.contactTarget === 'other') {
      if (!otherPerson.contactMethod) {
        errors.otherContactMethod = 'Please choose how I should contact them.';
      }

      const otherContactValueError = getContactValueError(otherPerson.contactMethod, otherPerson.contactValue ?? '');
      if (otherContactValueError) {
        errors.otherContactValue = otherContactValueError;
      }
    }
  }
...
export function Reception() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedDevice || Object.keys(validationErrors).length > 0 || isSubmitting) {
      return;
    }
  ...
  const submitErrorMessage = useMemo(() => {
    ...
    if (Date.now() - formStartTime < MIN_SUBMISSION_TIME_MS) {
      return 'Please take a moment to review your request before sending it.';
    }

    const lastSubmission = window.localStorage.getItem(LAST_SUBMISSION_KEY);
    if (lastSubmission && Date.now() - Number(lastSubmission) < SUBMISSION_RATE_LIMIT_MS) {
      return 'Please wait a short moment before sending another request.';
    }
              ...
              <div className="flex flex-col gap-10">
                ...
                <div className="border border-[var(--border-subtle)] p-6 md:p-8 bg-[var(--bg-base)]">
                  ...
                  <FieldError
                    ...
                    message={shouldShowError('description') ? validationErrors.description : ''}
                  />
                </div>
                ...
                <div className="border border-[var(--border-subtle)] p-6 md:p-8 bg-[var(--bg-base)]">
                  ...
                  <FieldError
                    ...
                    message={shouldShowError('contactMethod') ? validationErrors.contactMethod : ''}
                  />
                  ...
                  {contactMethod && (
                    <div className="mt-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                      ...
                      <FieldError
                        ...
                        message={shouldShowError('contactValue') ? validationErrors.contactValue : ''}
                      />
                    </div>
                  )}
                </div>
                  ...
                  <div className="border border-[var(--border-subtle)] p-6 md:p-8 bg-[var(--bg-base)] animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input
                          ...
                          className={getInputClassName(
                            shouldShowError('otherPersonName') && Boolean(validationErrors.otherPersonName)
                          )}
                        />
                        <FieldError
                          id="other-person-name-error"
                          message={shouldShowError('otherPersonName') ? validationErrors.otherPersonName : ''}
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <FieldError
                          ...
                          message={shouldShowError('relationship') ? validationErrors.relationship : ''}
                        />
                      </div>
                    </div>
                    ...
                    {otherPerson.contactTarget === 'other' && (
                      <div className="mt-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <FieldError
                          id="other-contact-method-error"
                          message={shouldShowError('otherContactMethod') ? validationErrors.otherContactMethod : ''}
                        />

                        {otherPerson.contactMethod && (
                          <div className="mt-6">
                            <label
                              htmlFor="other-contact-value"
                              className="block font-mono text-[10px] uppercase tracking-widest text-[var(--text-secondary)] mb-3"
                            >
                            ...
                            <input
                              ...
                              placeholder={
                                otherPerson.contactMethod === 'phone'
                                  ? 'Best number to reach them'
                                  : 'Best email to reach them'
                              }
                              ...
                              className={getInputClassName(
                                shouldShowError('otherContactValue') && Boolean(validationErrors.otherContactValue)
                              )}
                            />
                            <FieldError
                              id="other-contact-value-error"
                              message={shouldShowError('otherContactValue') ? validationErrors.otherContactValue : ''}
                            />
                          </div>
                        )}
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-t border-[var(--border-subtle)] pt-6">
                  <div className="max-w-xl">
                    <p className="text-sm text-[var(--text-secondary)] leading-relaxed">
                      I&apos;ll only use this information to help with your request.
                    </p>
