    ...
    >
      ...
      <AnimatePresence>
        {plant.harvestReady && (
          ...
          >
            <motion.g
              style={{ transformOrigin: '200px 530px' }}
              animate={{ scale: [1, 1.7, 1], opacity: [0.15, 0.5, 0.15] }}
              transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
            >
              <circle cx="200" cy="530" r={20} fill="none" strokeWidth="1" />
            </motion.g>
          </motion.g>
        )}
      </AnimatePresence>
      ...
      <motion.g
        animate={{ y: mood?.isRainy ? [0, -4, 0] : 0 }}
        transition={
          mood?.isRainy
          ...
          <AnimatePresence>
            {plant.harvestReady && fruits.map((fruit, i) => (
              ...
              >
                <motion.g
                  style={{ transformOrigin: `${fruit.x}px ${fruit.y}px` }}
                  animate={{ scale: [1, 1.444, 1], opacity: [0.08, 0.22, 0.08] }}
                  transition={{ duration: 2.8 + i * 0.3, repeat: Infinity, ease: 'easeInOut', delay: i * 0.4 }}
                >
                  <circle cx={fruit.x} cy={fruit.y} r={9} fill="none" strokeWidth={0.5} />
                </motion.g>
                <motion.circle
                  cx={fruit.x}
                  cy={fruit.y}
